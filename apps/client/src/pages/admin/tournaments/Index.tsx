...
export default function AdminTournaments() {
  ...
  return <AdminGuard><div className="mpl-page"><header className="mpl-topbar"><Link className="mpl-brand" to="/"><span className="mpl-mark">MPL</span><span>ADMIN · TOURNAMENTS</span></Link><Link className="mpl-link" to="/tournaments">Public Hub →</Link></header><main className="mpl-content"><div className="mpl-kicker">TOURNAMENT CONTROL PLANE</div><h1 className="mpl-title">Manage tournaments</h1><div className="mpl-actions"><button className="mpl-button" onClick={() => setShowCreate((value) => !value)}>Create tournament</button></div>{message && <div className="mpl-alert">{message}</div>}{showCreate && <form className="mpl-form" onSubmit={submit}><label>Tournament name<input value={name} onChange={(e) => setName(e.target.value)} required /></label><label>Slug<input value={slug} onChange={(e) => setSlug(e.target.value)} placeholder="m500-forbach-2026" required /></label><label>Level<select value={level} onChange={(e) => setLevel(e.target.value)}>{["M25","M50","M100","M250","M500","M1000"].map((item) => <option key={item}>{item}</option>)}</select></label><button className="mpl-button" type="submit">Create draft</button></form>}<section className="mpl-section"><div className="mpl-section-head"><h2>ALL TOURNAMENTS</h2><span>{items.length.toString().padStart(2, "0")}</span></div><div className="mpl-tournament-list">{items.map((item) => <div className="mpl-tournament-row" key={item.id}><Link to={`/tournaments/${item.slug}`}><div><span className="mpl-status">{item.status.replace("_", " ")}</span><h3>{item.name}</h3><p>{item.startDate ?? "Date TBC"}</p></div></Link><div className="mpl-actions"><strong>{item.level}</strong><Link className="mpl-link" to={`/admin/tournaments/${item.slug}/control`}>Control →</Link></div></div>)}</div></section></main></div></AdminGuard>;
}
